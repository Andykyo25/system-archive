...
**狀態**:DB migration 已套用正式庫;commit `d4a1e13` push main,Railway deploy Success,線上 /track 數字與本機一致、/rank 前段無 ETF(2026-09-23 實測)。

---

## 2026-09-23(續)— 成績單研究:勝率從哪裡來(純研究,未改任何選股邏輯)

方法:挑選當下即可得知的特徵(還原價 PIT 計算),**同日配對**(每天比「符合 vs 不符合」的平均超額,
抵銷當天大盤)+ **前後兩段**(H1 < 8/21 ≤ H2)都要同向才算。研究表已刪。

### 修正先前說法

- 「起漲掃描分數反向」**主要來自 8 月上半 7 天**。H2 分組平均差距縮到 1~2pp;配對後 ≥85 分 T+10 H1 −11.72 / H2 −1.56。
  → 方向微弱存在,幅度不可信。memory 與上方表格的「分數反向」要帶這個但書。

### 起漲掃描(scan,29 個掃描日)

| 條件 | T+10 上漲比例 | 配對:贏的天數 | H1 / H2 |
|---|---:|---:|---|
| 全部 | 39.9% | — | — |
| 漲停收(≥9.5%) | 46.5% | T+5 17/23、T+10 14/19 | 皆正 |
| 月線乖離 ≥15% | 48.4% | T+5 16/23、T+10 13/19 | 皆正 |
| **兩者皆否(非漲停且乖離<15%)** | **32.8%** | T+10 5/19 | 皆負 |
| 五條件全過 | 31.0% | T+10 3/13 | 皆負 |
| **漲停 且 距 60 日高 >15%** | **62.9%**(n=62,中位 +3.17%) | H2 T+5 14/15、T+10 11/11 | 皆正(H1 僅 3 天) |

- 五條件裡的「月線乖離 < 15%」正是被證據反對的那條 → passes_all 最差不是偶然
- **出場規則救不了勝率**:+8% 停利 / −8% 停損,T+10 勝率 39.7% → 41.0%,平均報酬 +0.71 → −0.88([[L52]] 再現)

### 回檔波段(swing)

- 熱股:T+20 配對 5/14、上漲 41% vs 73% → 排除
- 跌破月線(乖離<0)優於乖離 0~5%:T+5 21/31、T+20 12/19
- 60 日漲幅 >60%(強勢股回檔):T+5 14/19、T+20 6/7,上漲 74%
- 多因子排名 ≤50 **扣分**:T+20 −8.16pp、5/15

### 自己的買進(n=26)

- 追高(成交價 > 月線 +10%)T+10 上漲 78%(n=9)vs 未追高 43% —— 樣本太小,只記錄
- 4958 @500(9/22):當日 +4%、乖離 +0.4% → 落在「兩者皆否」組(歷史上漲比例約 33%)

### 最大的但書

全部樣本在同一段行情(0050 7/17 低點 → 9/18 +10% 的回升段)。「漲停 + 跌深」本質是 V 型反彈股,
正好吃回升行情的紅利,**換成空頭或盤整可能完全失效**。組合條件是看過資料後挑的(試了 8 條 + 1 組合),有多重比較偏誤。

### 事前登記(2026-09-23 鎖定,之後不得改參數)

- **H-A**:scan 候選中「漲停收(day_pct ≥ 9.5)且 close 距 60 日最高收盤 < −15%」的 T+10 超額,高於同日其他候選
- **H-B**:scan 候選中「day_pct < 9.5 且月線乖離 < 15%」的 T+10 超額,低於同日其他候選
- **H-C**:swing 非熱股中「60 日漲幅 > 60%」的 T+20 超額,高於同日其他非熱股
- 驗證樣本:只用 **2026-09-24 起**的挑選;判準:同日配對 T+10 贏的天數 ≥ 70%,且至少 **12 個不重疊視窗**(≈ 6 個月)
- 在驗證通過前:只當資訊標記,**不改 v_breakout_scan 計分、不當真錢下單依據**

---

## 2026-09-23(續2)— 介面改成「系統給結論」(Andy:太多備註與人工判讀,要中高信心看多、可直接進場驗證)

### 設計

- **信心由資料自動計算,不寫死**:scan 候選依型態分 A/B/C(A = 漲停且距 60 日高 >15%;B = 漲停或乖離 ≥15%;C = 其他)。
  每個型態的信心 = `mv_pick_scorecard` 中同型態**已到期**樣本的 T+10 上漲比例與筆數,每日隨 mv 刷新。
  門檻:上漲比例 ≥55% 且 n ≥30 才上榜。型態失效(比例掉下來)會**自動下榜**,不必人工調參 —— 這是對單一 regime 過擬合的防線。
- /scan 只剩:今日看多清單(每檔一句結論:型態、歷史勝率、參考價、停損、持有 10 日)+ 我的交易計畫
- 刪除:分數條、5 條件標籤、外資說明、免責段落、篩選器、統計 header、ObservationSummary(證據移到 /track)
- /track:刪說明段落,保留成績卡與清單,加「型態勝率」一張表

### 步驟

- [x] 1. mv_pick_scorecard 加 scan 型態欄(day_pct / 60 日高距離 / 乖離,PIT 計算)→ verify:A 組 T+10 n=62 / 上漲 62.9% 與研究一致
- [x] 2. `v_scan_verdict`:今日候選 × 型態統計 → 只輸出達門檻者 → verify:9/22 應為科嶠 / 統懋 / 漢磊
- [x] 3. /scan 重寫為結論卡;PlanForm 保留(收在按鈕後)→ verify:tsc / build / preview
- [x] 4. /track 精簡 → verify:preview
- rollback:migration 內保留舊 mv 定義;前端 git revert

### Review — 2026-09-23(Andy 選「只列中高信心」)

| # | 產出 | verify |
|---|---|---|
| 1 | `20260923000003_scan_verdict`:`scan_pattern_features` / `scan_pattern` 函式;mv 以 viewdef 包一層加 `day_pct/gap20/off_hi60/pattern` | mv 885 列不變;A 組 n10 **62 / 上漲 62.9%**,與研究逐位一致;`refresh concurrently` 成功 |
| 2 | `v_scan_pattern_stats`(最近 60 掃描日,≥55% 且 n≥30 = 中高)+ `v_scan_verdict` | 9/22 上榜 = 科嶠 / 統懋 / 漢磊(與預期相同);B 39% / C 33% 不列入;查詢 4.4s(主要是既有 v_breakout_scan,頁面快取 60s) |
| 3 | /scan → 「今日看多」結論卡(看多·信心 / 型態一句 / 買入·停損·持有 10 日 / 建立計畫收合);`planDefaults` 加 `antiChase` / `evidence` 選項;刪 ScanBoard / ObservationSummary / ScanCoverage;側欄改名「今日看多」 | 新增 1 個單元測試;preview:3 張卡、主內容約 390 字、表單預填 322.52–342.48 不受防追高截斷、375px 無橫向捲動 |
| 4 | /track:刪說明段落與註腳,分天期表 7 欄 → 5 欄,新增「型態勝率 · 決定今日看多」表;方法口徑移到 `lib/track.ts` 註解 | preview 頁面無 >30 字的說明段落 |

`tsc` / eslint / `next build` 通過;`npm test` 37/40(3 個既有 pglite 缺套件失敗,同前)。

**未處理**:`tests/ui-smoke.mjs`、`tests/scan-loading.mjs` 是 playwright fixture 測試,仍 mock `v_breakout_scan` 與舊 UI,
本機無 playwright 無法跑,**已過時需改寫**。`lib/scan.ts` 的 `summarizeObservations` 只剩單元測試在用(元件已刪)。

---

## 2026-09-23(續3)— 上榜後的風險閘門 + 盤中監控(Andy:漢磊上榜但盤中不該追、缺籌碼與套牢評估、要 5 分鐘盯一次並阻止進場)

### 證據(scan 候選,進場 = 訊號隔日收盤,同日配對 T+10 超額;研究表已刪)

| 條件 | n | 配對贏天數 | H1 / H2 | 10 日上漲 | 判斷 |
|---|---:|---:|---|---:|---|
| **上方套牢量 >30%**(過去 120 日在「現價~+20%」成交量佔比) | 112 | **1/18** | −7.50 / −4.79 | **29%** | ✅ 強且一致 → 做成閘門 |
| A 組 × 套牢 ≤30% | 47 | — | — | **72%**(中位 +8.9%) | |
| A 組 × 套牢 >30% | 15 | — | — | **33%**(中位 −0.9%) | 漢磊 = **46.6%** 屬此組 |
| 進場日追價(收 > 訊號收 +3%) | 93 | 10/19 | +2.67 / +3.19 | 43% | ❌ 不傷,反而略好 → **不擋** |
| 進場日長上影 | 103 | 10/16 | +6.82 / +3.95 | 47% | ❌ 不傷 → 不擋 |
| 跌回前 20 日高(突破失敗) | 164 | 9/19 | +7.90 / −4.43 | 39% | ⚠ 前後段相反,無證據 |
| 外資 5 日賣超 | 32 | 6/8 | +8.95 / +1.61 | 50% | ⚠ n 太小且方向相反;法人資料盤後才有 |

→ 修正先前說法:「今天盤中超過 71.59 不要追」**沒有資料支持**(追價組反而較好)。真正該擋漢磊的是套牢量。
→ 事前登記 **H-D**(2026-09-23 鎖定):A 組中套牢量 ≤30% 者 T+10 上漲比例高於 >30% 者;只用 9/24 起樣本驗證。

### 計畫

- [ ] 1. `scan_supply_share(symbol, date, price)` 函式;型態 A 改為「漲停 + 跌深 + 套牢 ≤30%」,其餘併入 B/C
      → verify:A 組統計 n=47 / 72%;9/22 上榜 = 科嶠、統懋(漢磊下榜)
- [ ] 2. `verdict_watch` 表:每日 mv 刷新後凍結當日上榜名單(停損價、買入區間);intraday EF 收料清單加入此表
      → verify:盤中 `price_intraday_cache` 有上榜股報價
- [ ] 3. `v_verdict_live`:上榜股 × 最新報價 → 狀態 🟢 可進場 / 🔴 停止(現價 ≤ 停損,或以現價重算套牢量 >30%)
- [ ] 4. /scan 卡片顯示即時狀態;🔴 時隱藏「建立交易計畫」並寫明原因;BuyForm 對 🔴 標的顯示紅色警告
- [ ] 5.(待 Andy 決定)Telegram:上榜股狀態轉 🔴 時推播,盤中每 5 分鐘檢查
- rollback:drop view/table/function;EF 還原前一版(部署前先存舊 content,[[L49]])

### Review — 2026-09-23(Andy 選「頁面擋單 + Telegram」、「追價/上影不擋」)

| # | 產出 | verify |
|---|---|---|
| 1 | `20260923000004`:`scan_supply_share`、4 參數 `scan_pattern`(A 需套牢 ≤30%,否則 D)、mv 加 `supply_share` | A 47 筆 / 72.3%、D 15 筆 / 33.3%;**漢磊 → D 下榜**;9/22 上榜 = 統懋(套牢 12.9%)、科嶠(9.3%) |
| 2 | `verdict_watch` + `freeze_verdict_watch()`(平日 15:45/22:45)+ `verdict_plan_levels` | freeze = 2 列;`20260923000005` 改 float 對齊後 **21 檔價位與 planDefaults 逐分一致** |
| 3 | `fetch-yahoo-intraday` v9 讀 `v_verdict_watch_symbols` | 盤中觸發:200、寫入 524(改版前 521-522);2434 / 4542 出現在 intraday cache |
| 4 | `v_verdict_live` + `verdict_watch_tick()`(盤中每 5 分鐘)→ Telegram(pg_net + vault,無新 EF) | 12:10 首跑成功:兩檔 ok(現價套牢 6.9% / 20.9%);block 訊息以模擬組字驗證中文 + emoji 正確;`20260923000006` 價格去尾零。**未實發測試推播**(runbook:不為驗證發測試訊息) |
| 5 | /scan 卡片:看多·可進場 / 停止進場 + 原因(隱藏建立計畫)、現價、上方套牢 %;BuyForm:停止 = 紅框、看多 = 綠框且不再誤報「盲區」 | preview 盤中實測兩張卡「可進場」;BuyForm 輸入 2434 顯示綠框 |

tsc / next build 通過;eslint 僅既有 3 warning;`npm test` 37/40(既有 pglite 失敗)。
**待觀察**:第一次真實 block 推播(需等名單股觸發);今天 15:45 第一次自動 freeze。
