...
## M11 大盤籌碼五指標驗證(2026-08-04,todo.md 內原記為 M10)

### L57 — 擇時訊號的檢定樣本必須跨 regime + 分年看穩健性;短樣本會製造「統計顯著」的假象
**問題**:驗 Andy 五條擇時指標時,先用手邊已有的 2 年樣本(2024-08~2026-08)跑,得到
「C 融資減 t=−3.80,顯著**反向**」—— 一個乾淨、漂亮、可以直接寫進結論的發現。
把樣本延長到 5 年(2021-08~2026-08,含 2022 空頭)後,**同一條訊號 t 掉到 −0.89**,
完全不顯著。反過來 D 費半從 t=0.70 變 t=2.59(看似顯著),但**分年一拆:六年裡四年為負**,
整體 t 全靠 2022/2025 兩年拉抬。兩個方向的錯誤結論都只差一步就會被我寫成定論。
根因:2024-08~2026-08 那 2 年基準 20 日報酬 +4.13%(年化 >50%)= **單一大多頭 regime**,
而這五條的本質是「**落底**訊號」—— 在一個沒有底部的樣本裡驗落底訊號,沒有檢定力可言。
**做法**:
1. **驗擇時/regime 類訊號前,先檢查樣本涵蓋哪些 regime**(至少一個完整空頭)。做法:
   先跑 benchmark 分年報酬,看到全是正的就知道這個樣本驗不動「落底」。這一步要在
   寫 spec 之前做,不是拿到結果才補([[L41]] 前提驗證前置的同一個道理)
2. **整體 t 值必須配分年 spread 一起看**。單一整體 t 值無法分辨「穩健 edge」與
   「兩個年份的極端值拉起來的雜訊」;分年表格是最便宜的 robustness 閘([[L36]])
3. 多重比較要記在心上:一次測 5 條訊號,α=0.05 的門檻實質要提到 |t|>2.58 左右。
   D 的 2.59 剛好卡在邊緣 = 「勉強擦線」而非「證明有效」
**為什麼**:[[L48]] 講的是前視偏誤造成的假 alpha,這條是它的孿生兄弟 ——
**樣本 regime 單一造成的假顯著**。兩者都會產出「數字漂亮、可以直接引用」的結論,
而且短樣本版本往往先做出來(手邊資料就那麼多),先入為主特別危險。
memory `stock_current_state` 的定調(不宣稱穩定 alpha)靠的就是這種自我拷問。

### L58 — 市場基準優先用報酬指數,別用個股/ETF 的未還原價;分割會產生 −66% 的假跌幅
**問題**:Phase 2 要一個 5 年的大盤基準,但 `price_daily` 的 0050 只回溯到 2023-01,
且帶 `adj_factor`。我判斷「補 2021-2023 進 price_daily 會在交界產生假跳空」(這個判斷對,
擋掉了一次 [[L45]] 等級的污染),於是改抓**未還原 close** 存進獨立表 —— 想法是
「反正只做組間相對比較,未還原也一致」。跑分年檢查時發現 **0050 在 2025 年顯示 −66.2%、
最低價 46.6**(前一年最高 202.8)。台股 2025 沒有這種跌幅:那是 **0050 分割**造成的
未還原價斷崖。若沒發現,分割日附近所有訊號都會被灌進一個 −75% 的假報酬。
**做法**:
1. **大盤基準一律優先用報酬指數**(FinMind `TaiwanStockTotalReturnIndex` data_id=TAIEX):
   含息、無分割、無 adj_factor 依賴,天然適合長期報酬比較。個股/ETF 的未還原價只適合
   看當日價位,**永遠不要拿來算跨期報酬**
2. 任何新接的價格序列,落地後先跑一次 sanity:`abs(單日變化) > 8%` 的天數 +
   分年報酬。台股有 10% 漲跌幅限制,指數單日超過 10% 幾乎必是資料問題
   (本次 TAIEX_TR 實測最大 ±9.7%,通過)
3. 「未還原價也一致所以沒關係」是錯的直覺:**一致的是價格單位,不是價格連續性**。
   除權息造成的偏差小(可忍受),分割造成的是斷崖(不可忍受),兩者要分開想
**為什麼**:[[L47]] 的精神(文件/直覺 ≠ 實際行為)在資料品質上的版本。這次是分年檢查
順手抓到的,但分年檢查本來是為了 L57 的目的才跑的 —— **例行的 sanity 掃描要獨立於
「這次分析需不需要」而存在**,否則就是靠運氣。

### L59 — 「合理的共線直覺」也要當假設驗證,不要寫進結論當前提
**問題**:提案階段我判斷「外資期貨回補 / 外資買超台積電 / 融資減,三者在下跌末段常同時發生,
高度共線,五條 AND 起來一年只觸發 1-2 次」,並用這個理由主張改做分數而非 AND 開關。
實測相關矩陣(n=1208):**fut vs tsmc r=0.056、fut vs margin r=0.017**,最高的一對也
只有 0.31 —— 五條彼此近乎獨立,共線疑慮完全不成立。順帶,score=5 實際出現 50 天
(不是我說的「一年 1-2 次」)。結論(改做分數)碰巧仍是對的,但**理由是錯的**。
**做法**:提案裡的「機制性直覺」(A 和 B 應該高度相關 / X 通常伴隨 Y)要標成**待驗假設**,
排進驗證清單,不要寫成論證前提。這類直覺很便宜就能驗(一個 corr 查詢)。
**為什麼**:[[L41]] 是「為案例設計 factor 前先驗證案例符合型態」,這條是它在**論證層**的
延伸 —— 用未驗證的機制說法去支持一個設計決策,即使決策對了,下次同樣的推理方式就會出錯。
共線與否還會直接影響結論解讀:本次因為五條獨立,「加總無預測力」才能歸因於
「每條本身沒訊號」,而不是「共線重複計票稀釋」。

---

## 起漲點掃描 backfill(2026-08-06)

### L60 — 回測前必查「樣本期間內每日的池子大小是否一致」;覆蓋不齊會讓結論整個反向
**問題**:替 v_breakout_scan 跑初步回顧,第一次得「訊號股 5 日 +1.96% vs 大盤 +0.38%,
超額 **+1.58pp**」—— 看起來這組型態條件有效。但當時 backfill 有洞:4/14-4/30 與
5/25-6/12 共 23 個交易日只有原本的 **180 檔**在池子裡(其餘日子是 1950 檔)。
補完洞重跑,同一組條件變成 **超額 −0.81pp、勝率 42.7%**,方向完全相反。
根因:池子小的那些日子,能入選的只有原本 universe 的大型權值股;池子大的日子才會
掃到中小型股。兩者混在同一個樣本裡,等於在不同母體上取樣後平均。
**做法**:
1. **跑任何橫斷面回測前,先查每個交易日的 symbol 數**(`count(*) group by trade_date`),
   確認全期一致。不一致就先補齊或把樣本截到一致的區間,**不要先跑再說**
2. backfill 完成後**必查洞**:`select trade_date, count(*) ... having count(*) < 預期`。
   本次 EF 回傳 `success=true` 但只處理了 12/30 天(TWSE 回應 4.3MB,解析吃 CPU,
   推測撞 EF 資源上限後靜默結束)——**「success 且無錯誤」不等於「跑完了」**
3. 分批 backfill 要小批(5-10 天)+ 迴圈查洞補跑,不能發一次就當完成
**為什麼**:[[L58]] 是價格序列本身的品質(分割斷崖),這條是**樣本覆蓋**的品質。
...
### L67 — 量測口徑必須對齊「它所服務的那個決策發生的時點」;用事後才存在的資訊衡量事前行為,不會報錯但結論是反的

同一天在交易行為分析裡抓到兩個 bug,型態完全一樣:

| 量測 | 用了什麼 | 服務哪個決策 | 那一刻真的有這資訊嗎 |
|---|---|---|---|
| `v_trade_behavior.dev_ma20_at_buy` | 進場日**收盤價** vs MA20 | BuyForm 下單當下的追高警示 | ❌ 收盤價那時還不存在 |
| `v_equity_curve` 拿來算回撤 | 只在**平倉事件**跳動 | 「我承受過多大風險」 | ❌ 抱單期間的浮虧完全不進曲線 |

兩者都不會報錯、數字都長得很合理,而且**都已經在生產上餵使用者做決策好幾個月**。

**後果不是誤差,是結論反向**:
- 追高:改用成交價後 17 筆只有 1 筆翻轉(2408 2026-06-05,dev 9.20→10.56),但那筆是
  +106,180 的最大單筆獲利 → 「追高組平均 11,549 / 非追高 33,078」變成
  「22,063 / 22,635」= **差異整個消失**。BuyForm 掛在上面的「別市價追」是建立在
  一筆分類錯誤上的規範性建議
- 回撤:階梯曲線只會在實現虧損時下降 → 顯示一路平順向上;真 MTM 是
  **峰值 530,636 @ 06-22、最大回撤 −30.22% @ 07-20**,且 06-22 到 08-17 的
  10 筆波段 + 2 筆當沖淨貢獻 **−329**(那兩個月主要是把 06-22 已存在的
  未實現 +140,500 換成現金,不是新獲利)。階梯曲線把「原地踏步」畫成「持續上升」

**做法**:
1. 任何量測欄位,寫下來時就要回答三題:**這個數字服務哪一個決策?那個決策發生在哪一刻?
   那一刻這份資料真的存在嗎?** 三題對不上就是口徑錯,不是精度問題
2. 特別警戒「**同一件事有兩條路徑各自算**」(這裡是 BuyForm 走 `v_entry_quality.dev_ma20_pct`
   即時價、行為 view 走收盤價)—— 兩條路徑量同一個概念卻取不同時點的價,是這類 bug 的溫床。
   同 [[L32]] 「線上 view vs backtest function 必須語意一致」,但那是**公式**一致,
   這條要求的是**資訊時點**一致
3. 事後分析用的「還原/彙總」序列(還原價、階梯權益、月頻彙總)拿來衡量**事前行為或風險**前,
   先問它平滑掉了什麼。階梯權益平滑掉的正是回撤本身 —— 用它算 MDD 等於用「只記錄成功」的
   log 算失敗率
4. 附帶:原始價與還原價混算也屬同一家族。修 `dev_ma20_at_buy` 時分子是成交價(原始)、
   分母 MA20 是還原價,不乘進場日 `adj_factor` 會差 0.3~0.8pp

**為什麼**:Andy 要 institutional-grade。這兩個 bug 的共通點是**它們讓系統看起來運作良好**——
一個把最大贏家歸到「你不該做的那類」、一個把 −30% 回撤畫成平線,方向都偏向「讓人安心」。
這是 [[L34]](查完整 code path 再下結論)、[[L38]](0 常是沒收集不是沒問題)、
[[L42]](沉默 drift)的同一家族,但前面幾條問的是「資料在不在」,這條問的是
**「資料存在,但它是那個時點的資料嗎」**。發現方式也一致:只有把口徑攤開跟它服務的決策
逐項對照才看得出來,靠抽查數字永遠看不出來 —— 因為每個數字單看都是對的。

**附註(工具坑)**:改這兩個 md 千萬別用 Git Bash `sed -i` —— 它讀 CRLF 寫 LF,會把整檔
...
### L71 — 優化前先遵循既有使用與部署決策
**問題**：在單人使用、已明定不做 Auth 的專案中，多加帳密與正式環境缺密碼即拒絕服務的條件，阻斷原有工作流程。
**做法**：本專案不新增網站帳號密碼。沿用使用者推送 GitHub、Railway 自動部署網站、Supabase 管理資料庫與 Edge Functions 的流程；網站建置不等於 Supabase 已更新。Supabase 服務金鑰與 EF 服務授權和網站登入是不同用途。
**驗證**：移除登入層後，用 production standalone 搭配隔離假資料確認頁面及 Server Actions 可直接使用，不能只靠 development 模式的例外判斷。

---

### L72 — 「上榜」不等於「可以進場」:結論型介面必須帶執行層閘門,而且每條閘門都要先過資料
**問題**(2026-09-23,Andy 糾正):/scan 改成只給「看多·信心中高」結論後,漢磊(3707)上榜;盤中我口頭說「不要追價」,但系統畫面沒有,Andy 表示照畫面就會直接買。事後驗證:漢磊上方套牢量 46.6%,屬於 A 組中上漲只有 33% 的那一群 —— 系統根本不該列出它。同時我口頭給的「超出買入區間不要追」與 Andy 直覺的「長上影要擋」,**資料都不支持**(追價組、長上影組 T+10 反而略好)。
**做法**:
1. 系統一旦宣稱「結論」,就要負責到「現在能不能買」:選股判斷(型態勝率)+ 執行閘門(停損、以現價重算的套牢量)都在後端算完,盤中定時更新並推播,不能留給人工判讀。
2. 每條擋單條件先做同日配對 + 前後兩段檢驗;只有兩段同向且多數天數成立的才做成閘門(本次只有「上方套牢量 >30%」過關,1/18 天較好)。直覺條件(追價、長上影、籌碼)沒過就不擋,但要把證據講清楚,連同修正自己先前的口頭建議。
3. 盤中狀態與前端價位要同一套算法(SQL `verdict_plan_levels` ↔ `planDefaults`,連 float 四捨五入都要對齊,21 檔逐分比對)。
**為什麼**:使用者要的是「系統說可以就能執行」。結論越乾淨,錯的結論傷害越大;漏掉的風險不會出現在畫面上,只會出現在帳上。

### L73 — MCP `apply_migration` 不會解 `\uXXXX`(`execute_sql` 會);含中文的 SQL 用 `U&'\XXXX'` 字面值機械產生
**問題**:照 L51 把 migration 轉成 JSON escaped 後貼進 `apply_migration`,結果 `\n` 與 `\uXXXX` 原樣送進 Postgres → 語法錯誤(整筆 rollback,無副作用)。同一招在 `execute_sql` 的小查詢卻會被解碼 —— 兩支工具行為不同,不能用一支的實測推論另一支。
**做法**:用腳本(scratchpad `uamp.cjs`)把 SQL 中含非 ASCII 的字串字面值轉成 PostgreSQL 原生 `U&'\XXXX'`(emoji 用 `\+XXXXXX`),含中文的整行註解移除,輸出純 ASCII 並做解碼往返比對(roundtrip = true)後再貼上。套用後用查詢結果(例:`reason` 欄位、模擬組出的推播文字)驗證中文正確。EF 則只剝除中文註解、確認程式碼行零非 ASCII 後部署,再做行為驗證(本次:觸發 EF 後看多名單出現在 `price_intraday_cache`)。
**為什麼**:L49 的核心是「中文不經我重打」;工具的 escape 行為要逐支實證([[L47]]),最穩的是讓要貼的內容本身就是 ASCII。
