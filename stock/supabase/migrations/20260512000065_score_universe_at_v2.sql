-- M9.1: score_universe_at v2 — 同步 17 factor + settings-aware weights
--
-- 變動對應 v_factor_scores v2 + v_stock_rank v2:
--   * 新增基本面 factor fund_gross_up(gross_margin_yoy_pp > 0)
--   * RSI factor 改成 mom_rsi_strong(>50,從 v1 <70 邏輯改)
--   * 新增籌碼 factor chip_inst_concentration(3 日法人 net / 3 日總量 > 5%)
--   * fund_peg_low 用 app_settings.peg_threshold(預設 1.2)
--   * weights 4 維度從 app_settings 拿(weight_fund/mom/rev/chip,預設 0.40/0.30/0.10/0.20)
--
-- 為什麼 function 也要改:
--   M10 backtest harness 走 score_universe_at,如果這個 function 還跑舊版邏輯,
--   backtest 結果跟線上 view 不一致,backtest 結論失準。spec 強制要求同步。
--
-- 注意:
--   stable function 內讀 app_settings 是 OK 的(stable 不是 immutable),
--   雖然 backtest 一輪 rebalance 跑 N 次 score_universe_at 各拿一次 settings(有 overhead),
--   但 N 通常 << 200,且 settings 表只有 ~9 row,讀取極快。
...
chip as (
  select
    ...
    -- chip_inst_concentration:join inst three_major_net_3d_sum 與 price volume_3d_sum
    -- 注意:price 的 inner join 才能拿到 vol_days_available + volume_3d_sum,
    --     所以這裡先放 inst 提供 net,price subselect 用 universe.symbol 在最終 combined 階段對齊
...
scored as (
  select
    ...
    coalesce(case when c.mom_ma_golden = 1 then 1 else 0 end, 0)
      ...
      + coalesce(case when c.mom_rsi_strong = 1 then 1 else 0 end, 0) as mom_count_pos_v,
    (case when c.mom_ma_golden is not null then 1 else 0 end
      + case when c.mom_ret_diff is not null then 1 else 0 end
      + case when c.mom_rsi_strong is not null then 1 else 0 end) as mom_count_total_v,
    -- rev 2
...
comment on function public.score_universe_at(date) is
  'M9.1 v2:對齊 v_factor_scores v2 + v_stock_rank v2(17 factor + settings-aware weights/peg_threshold)。
   stable function 內讀 app_settings,settings 改變後新 backtest 即生效。';
