-- M9.4c 階段 3 Step 1:score_universe_at 同步加 mom_strong_persist(2026-05-16)
--
-- L32 鐵律:v_price_factors 加了第 6 個動能 factor mom_strong_persist
--   (20260515000006),backtest 路徑 score_universe_at 必須同步,否則
--   線上排名用 6 動能、backtest 用 5 動能,OOS 結論失準。
--
-- 本次改點(其餘維持 Step 0 + 0.5,byte 不動):
--   1) price CTE 加 mom_strong_persist:ret_60d>0 AND close>MA20 AND
--      RSI14 50~75,日數保護 RSI≥13/MA20≥20/需 60d 前價。完全鏡像
--      v_price_factors 20260515000006(同算式、同 RSI 邊界處理)。
--   2) combined 帶 p.mom_strong_persist
--   3) scored mom_count_pos_v / mom_count_total_v 動能 5→6 對應
--   point-in-time(period_end/trade_date<=as_of_date)維持(L26)。
--   Step 0 的 median PEG + 0.5 的 close*adj_factor 全保留不動。
--
...
price as (
  select
    ...
    -- factor mom_strong_persist(M9.4c:ret_60d>0 + close>MA20 + RSI14 50~75)
    --   L32 鏡像 v_price_factors 20260515000006(同日數保護、同 RSI 算式)
    case
      when a.rsi_days_available is null or a.rsi_days_available < 13 then null::int
      when a.ma20_days_available is null or a.ma20_days_available < 20 then null::int
      when a.close_60d_ago is null or a.close_60d_ago <= 0
        or a.latest_close_v is null or a.ma20_now is null then null::int
      when ((a.latest_close_v - a.close_60d_ago) / a.close_60d_ago) > 0
        and a.latest_close_v > a.ma20_now
        and (case
               when a.loss_14 = 0 and a.gain_14 = 0 then 50
               when a.loss_14 = 0 then 100
               else 100 - (100.0 / (1 + (a.gain_14 / a.loss_14)))
             end) between 50 and 75 then 1
      else 0
...
combined as (
  select
    ...
    p.mom_breakout, p.mom_above_ma200, p.mom_strong_persist,
...
scored as (
  select
    ...
    coalesce(case when c.mom_ma_golden = 1 then 1 else 0 end, 0)
      ...
      + coalesce(case when c.mom_strong_persist = 1 then 1 else 0 end, 0) as mom_count_pos_v,
    (case when c.mom_ma_golden is not null then 1 else 0 end
      ...
      + case when c.mom_strong_persist is not null then 1 else 0 end) as mom_count_total_v,
    -- rev 2
...
comment on function public.score_universe_at(date) is
  'M9.4c 階段 3 Step 1:加 mom_strong_persist(動能 6 條)。含 Step 0 median
   PEG 對齊 + 0.5 close*adj_factor。對齊 v_factor_scores/v_stock_rank
