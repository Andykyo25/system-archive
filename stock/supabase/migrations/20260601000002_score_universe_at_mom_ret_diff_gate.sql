-- A2(全系統審查):score_universe_at.mom_ret_diff 加 ret_60d>0 gate(2026-06-01)
--
-- 配對 20260601000001(v_price_factors 同一修正)。L32 鐵律:回測路徑
-- 必須與線上 v_price_factors/v_factor_scores 用同一 mom_ret_diff 定義,
-- 否則破壞「回測=線上」一致性(審查確認的關鍵優點)。
--
-- 唯一變更:mom_ret_diff case 加 `and ret_60d > 0`(排除跌深反彈假動能)。
-- body 其餘與 20260519000005 逐字相同(動能 5 條 / median PEG / 0.5
-- close*adj_factor / point-in-time)。
--
-- 影響:會改變歷史回測 top5 成分(跌深反彈股不再計入 mom_ret_diff)→
--   +24.19 OOS 基準會更新成更誠實的數字。apply 後須重跑 2025 OOS 對照。
...
comment on function public.score_universe_at(date) is
  'A2(20260601):mom_ret_diff 加 ret_60d>0 gate(排除跌深反彈假動能),
   與 v_price_factors 20260601000001 同步(L32)。body 其餘 = 20260519000005
   逐字(動能 5 條 / median PEG / 0.5 close*adj_factor / point-in-time)。';
