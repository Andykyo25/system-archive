-- M9.4c REVERT:score_universe_at 回 Step 0 版(動能 5 條,2026-05-16)
--
-- 原因:M9.4c(mom_strong_persist)OOS 驗證惡化(top5 2025 +17.75→+10.42),
--   Andy 拍板 revert。本檔 = Step 0(20260515000005)body 原封不動重貼
--   (動能 5 條、median PEG 對齊 v_stock_score、0.5 close*adj_factor、
--   point-in-time L26),只是退掉 20260515000009 加的 mom_strong_persist。
--
-- 為什麼用整段重貼而非 diff:score_universe_at 是 function,直接 create
--   or replace 回 Step 0 內容最乾淨可審。L32:對應 v_factor_scores
--   20260515000010(mom_count 同步回 5)。
--
-- 驗證:score_universe_at(today) vs v_factor_scores 全 universe 四維
--   count 0 不一致(mom_total 回 5);4 個 backtest byte 回 Phase 0.7
--   (2024 t10 -35.23/t5 -22.08;2025 t10 +8.41/t5 +17.75)。
