-- C1+C2(全系統審查):score_universe_at 修前視偏誤 + 事後選擇偏誤(2026-06-01)
--
-- C1 財報/月營收前視偏誤(P0):原 fund_ranked `period_end <= as_of_date`、
--   rev_ranked `period<=as_of月` = 用「會計期間」當可得日,等於在公告前就偷看
--   財報/營收(實證 2330 在 4/30 就「看到」Q1,實際 ~5/15 才公告)。
--   修:加保守發布落差 —
--     季報:Q1-Q3 period_end+45天(法定 ~5/15、8/14、11/14)、Q4 年報+90天(隔年3/31)
--     月營收:該月月初+40天 ≈ 次月10日(法定公告日)
--
-- C2 universe 事後選擇偏誤(P0):原 universe 含 watchlist + holdings_transactions
--   = Andy 事後才關注/買進的贏家名單,灌進歷史選股池 = hindsight。
--   修:只留可機械篩的 stock_universe ∪ industry_stocks ∪ etf_metadata。
--
-- base = 20260601000002(A2 版,含 mom_ret_diff ret_60d>0 gate)逐字,改 3 處
--   (universe_symbols / fund_ranked where / rev_ranked where)。
--
-- 影響:會再降 OOS alpha(揭露真實 edge)。apply 後重跑 2025 OOS 對照 +20.02。
--   這是「誠實化」非「調好看」,符合 L36 透明原則。
...
with universe_symbols as (
  -- C2:移除 watchlist + holdings_transactions(事後才進視野的贏家名單 = 事後選擇
  --   偏誤),回測只用當時可機械篩出的選股池 stock_universe ∪ industry ∪ etf。
...
price_ranked as (
  ...
  where p.trade_date <= as_of_date
    and p.trade_date > as_of_date - interval '320 days'
),
price_agg as (
  select
    symbol,
...
price as (
  select
    ...
    -- factor mom_ret_diff(A2:加 ret_60d>0 gate,排除跌深反彈假動能)
    case
      ...
      when ((a.latest_close_v - a.close_20d_ago) / a.close_20d_ago) > 0
        and ((a.latest_close_v - a.close_60d_ago) / a.close_60d_ago) > 0
...
fund_ranked as (
  ...
  -- C1:財報發布落差(避免偷看未公告財報)。台股法定:Q1~5/15、Q2~8/14、Q3~11/14
  --   (period_end+~45天);Q4 年報隔年 3/31(period_end+90天)。
  where (case when extract(month from period_end) = 12
              then period_end + interval '90 days'
...
rev_ranked as (
  ...
  -- C1:月營收次月 10 日公告。period(M月,以月初代表)+40天 ≈ M+1 月 10 日,
  --   as_of 過此才可用 M 月營收(避免偷看當月未公告營收)。
  where make_date(period_year, period_month, 1) + interval '40 days' <= as_of_date
),
rev_agg as (
  select
    symbol,
...
comment on function public.score_universe_at(date) is
  'C1+C2(20260601):C1 財報/月營收加發布落差(季報+45天/年報+90天/月營收次月10日)
   修前視偏誤;C2 universe 移除 watchlist+holdings_transactions 修事後選擇偏誤。
   含 A2 mom_ret_diff ret_60d>0 gate。揭露真實 OOS alpha(較 +20.02 更低)。';
