-- Phase 0.5:score_universe_at 技術因子改用還原權值 adj_close(2026-05-15)
--
-- L32 鐵律:v_price_factors(線上「現在」視角)與 score_universe_at
--   (backtest「歷史」視角)是兩條獨立計算路徑,改一條必須同步改另一條,
--   否則 backtest 用舊邏輯、線上用新邏輯,結論失準且難 debug。
--   本檔對應 20260515000002_v_price_factors_adj。
--
-- 鐵律(Phase 0):不動 factor 邏輯/權重/門檻/top_n,只把取價
--   raw close → close * adj_factor。volume 維持 raw。
-- 唯一改點:price_ranked CTE 把 p.close 換成 p.close * p.adj_factor
--   (alias 仍叫 close),price_change 同步。下游 price_agg/price 全用
--   alias close,0 改動。
--
-- 退版保險:adj_factor 全設 1.0 時 close*1.0 數值恆等 raw close,
--   此 function 行為 byte 級 = 前版(20260513000001_score_universe_at_v4_window_fix)。
...
comment on function public.score_universe_at(date) is
  'Phase 0.5:技術因子取價改 close*adj_factor(adj_close),其餘對齊
   v_factor_scores v4 + v_stock_rank v4(19 factor + mom_above_ma200)。
   L32 同步對應 v_price_factors(20260515000002)。
