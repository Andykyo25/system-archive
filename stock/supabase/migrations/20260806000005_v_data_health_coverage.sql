-- v_data_health v3:加 coverage 檔數腰斬偵測 + scan_picks 新鮮度(2026-08-06)
--
...
fresh_raw as (
  ...
  union all
  -- 起漲掃描前向凍結:只在當日有 80 分以上標的時寫入,市場極弱可能連續數日無 → 門檻放寬
...
-- ── B2. 收盤價涵蓋檔數(檔數腰斬偵測)────────────────────────────
-- freshness 只看 max(trade_date),抓不到「日期有更新但檔數腰斬」——
-- 2026-08-04 上櫃只進 25 檔(正常約 870)卻全程無警報,是沉默 drift 第四次重演。
cov as (
...
cov_stat as (
  select
    ...
    (select percentile_cont(0.5) within group (order by n) from cov)::numeric as median_n
),
coverage_rows as (
  select
