-- M9.5 REVERT:score_universe_at 回最新有效基線(動能 5 條,2026-05-19)
--
-- 用途:備用 revert。若 mom_low_base_breakout(M9.5)OOS 驗證惡化,執行
--   本檔把 backtest 路徑回 Phase 0.7 基準(動能 5 條),與 v_factor_scores
--   20260519000004 同步(L32)。
--
-- 來源:本檔 body = 20260515000011(score_universe_at M9.5 之前最新有效版,
--   M9.4c REVERT 後動能 5 條、median PEG 對齊 v_stock_score、0.5
--   close*adj_factor、point-in-time L26)原封不動逐字重貼 —
--   等同把 20260519000003(forward)的三處 append 全部退掉
--   (price_agg low_120d / price CTE mom_low_base_breakout / mom_count 6→5)。
--   除檔頭與檔尾 comment 字串外,函式 body 與 20260515000011 逐字相同。
--   v_price_factors 的 low_120d / mom_low_base_breakout 欄留著(append-only,
--   行為自動回 — backtest 路徑不再讀計分,線上 v_factor_scores 由
--   20260519000004 同步降回不計分 dead 欄)。
--
-- 驗證:score_universe_at(today) vs v_factor_scores(20260519000004)全
--   universe 四維 count 0 不一致(mom_total 回 5);4 個 backtest byte
--   回 Phase 0.7(2024 t10 -35.23/t5 -22.08;2025 t10 +8.41/t5 +17.75)。
...
comment on function public.score_universe_at(date) is
  'M9.5 REVERT:回 20260515000011 基線(動能 5 條)。body 與 20260515000011
   逐字相同 — 退掉 20260519000003 的三處 append。含 Step 0 median PEG 對齊
   + 0.5 close*adj_factor。對齊 v_factor_scores 20260519000004(19 計分)。
   point-in-time 保留。讀 app_settings(peg1.5/roe10/4w)。';
