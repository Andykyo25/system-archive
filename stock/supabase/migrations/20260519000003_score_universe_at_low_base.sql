-- M9.5 階段 1 Step 1:score_universe_at 同步加 mom_low_base_breakout(2026-05-19)
--
-- L32 鐵律:v_price_factors 加了第 6 個動能 factor mom_low_base_breakout
--   (20260519000001),backtest 路徑 score_universe_at 必須同步,否則
--   線上排名用 6 動能、backtest 用 5 動能,OOS 結論失準。
--
-- 來源 & 改動範圍:本檔 = 20260515000011(score_universe_at 目前最新有效版,
--   M9.4c REVERT 後動能 5 條、median PEG 對齊 v_stock_score、0.5 close*adj_factor、
--   point-in-time L26)body 逐字複製,只做三處 append:
--   (a) price_agg CTE 尾端加 low_120d + low_120d_days_available(120 日最低
--       close,close 已乘 adj_factor,與 high_20d 同源 — Phase 0.5)
--   (b) price CTE 加 mom_low_base_breakout(低基期<25% + 帶量 1.4x + 20 日新高,
--       L23 日數保護 MA20≥20/需滿 120 筆/high_20d·vol 非 null);combined
--       帶 p.mom_low_base_breakout
--   (c) scored mom_count_pos_v / mom_count_total_v 動能 5→6 對應
--   除這三處外,其餘每一行與 20260515000011 逐字相同 —
--   禁止任何重構/改格式/改其他 factor/改 PEG/改 point-in-time 邏輯/改視窗天數
--   (Phase 0 鐵律 + L32 同步 + L37)。20260515000011 不含 mom_strong_persist
--   (REVERT 已退掉),本檔維持不加回。
--
-- LOW_BASE_PCT (掃描用,Gate2 會改此值):門檻 25 先 hardcode,日後掃描調此數重 apply。
--
-- 驗證:score_universe_at(today) vs v_factor_scores 全 universe
--   fund/mom/rev/chip 四維 count 0 不一致(mom_total 變 6)。
--   接著跑 2024 in + 2025 OOS × top5/top10 對比 Phase 0.7 基準定 alpha。
...
settings as (
  select
    coalesce((select value from public.app_settings where key = 'peg_threshold'), 1.5::numeric) as peg_threshold,
...
-- ====================== Price ranked (320 day window; adj_close) ======================
price_ranked as (
  select
  ...
  where p.trade_date <= as_of_date
    and p.trade_date > as_of_date - interval '320 days'  -- M9.3:擴大視窗給 MA200
),
price_agg as (
  select
    symbol,
    ...
    avg(case when rn between 1 and 200 then close end) as ma200_now,  -- M9.3 NEW
    ...
    count(case when rn between 1 and 200 then 1 end) as ma200_days_available,  -- M9.3 NEW
    -- M9.5:append-only 尾端 — 120 日最低 close(已乘 adj_factor)+ 資料量
    min(case when rn between 1 and 120 then close end) as low_120d,
    count(case when rn between 1 and 120 then 1 end) as low_120d_days_available
  from price_ranked
  group by symbol
),
price as (
  select
    ...
    case
      ...
      when a.loss_14 = 0 and a.gain_14 = 0 then 0
      when a.loss_14 = 0 then 1
      when (100 - (100.0 / (1 + (a.gain_14 / a.loss_14)))) > 50 then 1
      else 0
    end as mom_rsi_strong,
    -- factor mom_breakout(M9.2:20 日新高 + 放量 2x)
    case
    ...
    -- factor mom_above_ma200(M9.3 NEW:站上 MA200 + 60 日報酬正)
    case
      when a.ma200_days_available is null or a.ma200_days_available < 180 then null::int
      when a.ma200_now is null or a.latest_close_v is null or a.close_60d_ago is null
        or a.close_60d_ago <= 0 then null::int
      when a.latest_close_v > a.ma200_now
        and ((a.latest_close_v - a.close_60d_ago) / a.close_60d_ago) > 0 then 1
      else 0
    end as mom_above_ma200,
    -- M9.5 factor mom_low_base_breakout(低基期<25% + 帶量 1.4x + 20 日新高)
    --   L23 日數保護比照 mom_breakout/mom_strong_persist:MA20≥20、需滿 120
    --   筆可算 low_120d、high_20d/vol 非 null,任一不足 → null::int
    case
      when a.ma20_days_available is null or a.ma20_days_available < 20 then null::int
      when a.low_120d_days_available is null or a.low_120d_days_available < 120 then null::int
      when a.low_120d is null or a.low_120d <= 0
        or a.high_20d is null or a.latest_close_v is null
        or a.vol_latest is null or a.vol_5d_avg is null or a.vol_5d_avg = 0 then null::int
      when ((a.latest_close_v - a.low_120d) / a.low_120d) * 100 < 25  -- LOW_BASE_PCT (掃描用,Gate2 會改此值)
        and a.vol_latest > a.vol_5d_avg * 1.4
        and a.latest_close_v >= a.high_20d * 0.99 then 1
      else 0
    end as mom_low_base_breakout,
    -- factor rev_off_high
    case
      when a.ma60_days_available is null or a.ma60_days_available < 60 then null::int
...
-- ===== Step 0:鏡像 v_stock_score 的 4 季 EPS YoY 中位數 =====
-- 對 q1..q4 各算 (q[i]-q[i+4])/abs(q[i+4])*100,filter not null 後取 median。
-- 完全對應 migration 20260512000003 的 quarterly_yoys + quarterly_yoy_median。
qyoy_median as (
  select symbol,
    percentile_cont(0.5) within group (order by yoy_value) as median_yoy
  from (
    select symbol, yoy_value from (
      select symbol, case when eps_q5 is not null and abs(eps_q5) > 0
        then (eps_q1 - eps_q5) / abs(eps_q5) * 100 end as yoy_value from fund_agg
    ) q where yoy_value is not null
    union all
    select symbol, yoy_value from (
      select symbol, case when eps_q6 is not null and abs(eps_q6) > 0
        then (eps_q2 - eps_q6) / abs(eps_q6) * 100 end as yoy_value from fund_agg
    ) q where yoy_value is not null
    union all
    select symbol, yoy_value from (
      select symbol, case when eps_q7 is not null and abs(eps_q7) > 0
        then (eps_q3 - eps_q7) / abs(eps_q7) * 100 end as yoy_value from fund_agg
    ) q where yoy_value is not null
    union all
    select symbol, yoy_value from (
      select symbol, case when eps_q8 is not null and abs(eps_q8) > 0
        then (eps_q4 - eps_q8) / abs(eps_q8) * 100 end as yoy_value from fund_agg
    ) q where yoy_value is not null
  ) u
  group by symbol
),
-- ===== Step 0:鏡像 v_stock_score 的 peg 三層 fallback =====
-- median_yoy>0 → pe/median_yoy ; eps_q5>0 且 eps_q1>eps_q5 → pe/last_q ;
-- eps_ttm_prev>0 且 eps_ttm>eps_ttm_prev → pe/ttm ; 皆不符 → null。
-- 無 ELSE(對應 v_stock_score CASE 無 else = null)。
peg_cte as (
  select fa.symbol,
    case
      when lv.pe > 0 and qm.median_yoy is not null and qm.median_yoy > 0
        then lv.pe / qm.median_yoy
      when lv.pe > 0 and fa.eps_q5 > 0 and fa.eps_q1 > fa.eps_q5
        then lv.pe / (((fa.eps_q1 - fa.eps_q5) / fa.eps_q5) * 100)
      when lv.pe > 0 and fa.eps_ttm_prev > 0 and fa.eps_ttm > fa.eps_ttm_prev
        then lv.pe / (((fa.eps_ttm - fa.eps_ttm_prev) / fa.eps_ttm_prev) * 100)
    end as peg
  from fund_agg fa
  left join latest_valuation lv on lv.symbol = fa.symbol
  left join qyoy_median qm on qm.symbol = fa.symbol
),
fund as (
  select
    fa.symbol,
    ...
    -- Step 0:鏡像 v_factor_scores 對 ss.peg 的判定(peg 來自 peg_cte = v_stock_score 邏輯)
    case
      when pc.peg is null then null::int
      when pc.peg > 0 and pc.peg < s.peg_threshold then 1
      else 0
    end as fund_peg_low,
    case
  ...
  left join peg_cte pc on pc.symbol = fa.symbol
  left join rev_agg ra on ra.symbol = fa.symbol
  cross join settings s
),
-- ====================== Combine -> 19 factor int + count ======================
combined as (
  select
    u.symbol,
    f.fund_eps_pos, f.fund_eps_yoy, f.fund_roe_high,
    f.fund_fcf_pos, f.fund_peg_low, f.fund_rev_yoy, f.fund_gross_up,
    p.mom_ma_golden, p.mom_ret_diff, p.mom_rsi_strong,
    p.mom_breakout, p.mom_above_ma200, p.mom_low_base_breakout,
...
scored as (
  select
    ...
    -- mom 6(M9.5:加 mom_low_base_breakout)
    coalesce(case when c.mom_ma_golden = 1 then 1 else 0 end, 0)
      ...
      + coalesce(case when c.mom_breakout = 1 then 1 else 0 end, 0)
      + coalesce(case when c.mom_above_ma200 = 1 then 1 else 0 end, 0)
      + coalesce(case when c.mom_low_base_breakout = 1 then 1 else 0 end, 0) as mom_count_pos_v,
    (case when c.mom_ma_golden is not null then 1 else 0 end
      ...
      + case when c.mom_breakout is not null then 1 else 0 end
      + case when c.mom_above_ma200 is not null then 1 else 0 end
      + case when c.mom_low_base_breakout is not null then 1 else 0 end) as mom_count_total_v,
    -- rev 2
...
comment on function public.score_universe_at(date) is
  'M9.5 階段 1 Step 1:加 mom_low_base_breakout(動能 6 條)。= 20260515000011
   body 逐字 + 三處 append(low_120d / mom_low_base_breakout / mom_count 5→6)。
   含 Step 0 median PEG 對齊 + 0.5 close*adj_factor。對齊 v_factor_scores
   (20 factor:7+6+2+5)。point-in-time 保留。讀 app_settings(peg1.5/roe10/4w)。';
