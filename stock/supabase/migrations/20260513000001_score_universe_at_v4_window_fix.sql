-- M9.3 hotfix:score_universe_at window 220 → 320(同 v_price_factors fix)
-- 原 220 calendar days × 5/7 = 155 trade days < 180 → mom_above_ma200 永遠 null
-- backtest 歷史視角也受影響,跟線上 view 必須同步(L32)

-- M9.3: score_universe_at v4 — 同步 19 factor + mom_above_ma200
--
-- 變動對應 v_factor_scores v4 + v_stock_rank v4:
--   * 新增動能 factor mom_above_ma200(close > MA200 + 60d ret > 0)
--   * price agg 加 ma200_now + ma200_days_available 計算(rn between 1 and 200)
--   * count_pos / count_total 動能 4 → 5 對應
--   * price_ranked 視窗從 180 → 220 天(MA200 需要 200 個交易日)
--   * settings.peg_threshold 預設改 1.5(M9.3),roe_threshold 預設仍 10
--
-- 為什麼 function 也要改:L32 強制 — view 是線上「現在」視角,
--   function 是 backtest「歷史」視角,兩條邏輯必須語意一致才能 trust backtest。
...
-- ====================== Price ranked (220 day window for MA200) ======================
price_ranked as (
  select
...
comment on function public.score_universe_at(date) is
  'M9.3 v4:對齊 v_factor_scores v4 + v_stock_rank v4(19 factor + mom_above_ma200)。
   stable function 內讀 app_settings(peg_threshold 預設 1.5 / roe_threshold 預設 10 / 4 個 weights)。';
