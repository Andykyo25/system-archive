-- Expose ATR14 so the plan form can derive a stop from the EXISTING
-- atr_stop_multiple setting instead of inventing a distance. Scoring weights,
-- column order and every existing column are unchanged; atr14 is appended.
-- Roll back by re-applying 20260906000001_scan_adjusted.sql.
...
normalized as (
  select p.*, p.close * p.adj_factor as adj_close,
         ...
         p.low * p.adj_factor as adj_low
  from public.price_daily p, bounds b
  where p.trade_date >= b.from_d and p.close > 0 and p.adj_factor > 0
),
px as (
  select
    p.symbol, p.trade_date, p.adj_close as close, p.adj_high as high,
    p.adj_low as low, p.volume,
...
chg as (
  select px.*,
    ...
    -- True range on the SAME adjusted basis as every other technical column.
    case when prev_close is null or high is null or low is null then null
...
rsi_calc as (
  select chg.*,
    ...
    avg(tr)   over w14 as atr14_adj,
    count(tr) over w14 as n_atr,
...
metrics as (
  select
    ...
    -- Incomplete windows stay null; a short-history stop must not be guessed.
    case when t.n_atr >= 14 then round(t.atr14_adj / t.adj_factor, 2) end as atr14,
    t.n_bars, t.n_slope
  from today t
...
comment on view public.v_breakout_scan is
  'breakout-v3-adjusted: unchanged scoring weights; adjusted technical prices converted back to signal-day price units; 20 prior bars; per-symbol five-session chips; atr14 (14 full bars required) for stop sizing.';

-- Slippage is a modelling assumption for the sizing estimate, not an allocation
-- limit. Kept in settings so it is changed in one place, not per form.
insert into public.app_settings (key, value, description)
values ('plan_slippage_pct', 0.3,
  '交易計畫股數估算的單邊滑價假設(%)。只影響估算,不改變任何已保存計畫。')
