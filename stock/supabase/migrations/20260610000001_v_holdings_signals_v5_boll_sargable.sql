-- v_holdings_signals v5 — boll CTE sargable(2026-06-10,純效能零邏輯改動):
--   v4 的 boll 內層掃 price_daily 只有 close>0 filter,planner 走全表 Seq Scan
--   12.8 萬列再 window rn<=20 — 實測整條 view 941ms,是 /holdings 與 TG 推播最大瓶頸。
--   修法同 20260602000003(v_latest_price_realtime sargable):加 trade_date 90 天下界,
--   rn<=20 只需 ~20 個交易日(90 個日曆天含長假絕對足夠)→ 結果集不變,改走 PK index。
--   迴歸:apply 後 EXCEPT 雙向 diff = 0(L48,自己 DB 內驗)。
--
...
boll as (
  ...
  from (
    ...
    where symbol in (select symbol from base) and close > 0
      -- v5 sargable:rn<=20 只需近 20 交易日,90 日曆天下界讓 planner 走
      -- (symbol, trade_date) PK index 而非全表 Seq Scan(實測 941ms 主因)
...
comment on view public.v_holdings_signals is
  '持股訊號規則層 v5(2026-06-10):
   - v5:boll CTE 加 trade_date 90 天 sargable 下界(純效能,結果集不變)
   v4(2026-06-02):
