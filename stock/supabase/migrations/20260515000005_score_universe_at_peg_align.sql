-- M9.4 階段 3 Step 0:score_universe_at 的 fund_peg_low 對齊 v_stock_score(2026-05-16)
--
-- 背景(L32 違反根因,實證):線上 fund 7 factor 經 v_stock_score 算;
--   backtest score_universe_at 是 inline 重算。實測 2376/2492 唯一分歧 =
--   fund_peg_low,且方向相反 → 不是門檻偏移,是 PEG 公式根本不同:
--   * 線上 v_stock_score.peg:3 層 fallback,主用「4 季 EPS YoY 中位數」
--     (median 抗 outlier,migration 20260512000003 刻意設計)
--   * 舊 backtest:只用單季 (eps_q1-eps_q5)/eps_q5,無 median、無 ttm fallback
--   → 含 PEG 的 backtest 結論有系統偏差,M9.4 階段 3 OOS 前必修。
--
-- 本次唯一改點(其餘 byte 不動,含 0.5 的 close*adj_factor):
--   1) fund_agg 補 eps_q2..q4 / eps_q6..q8(算 4 季 quarterly YoY)
--   2) 新增 qyoy_median CTE:percentile_cont(0.5) over 非 null 的 yoy_q1..q4
--      (完全鏡像 v_stock_score quarterly_yoy_median)
--   3) 新增 peg_cte:peg = pe/median_yoy → pe/last_q → pe/ttm 三層 fallback
--      (完全鏡像 v_stock_score peg)
--   4) fund_peg_low 改為:peg null→null;peg>0 且 peg<peg_threshold→1;else 0
--      (完全鏡像 v_factor_scores 對 ss.peg 的判定)
--   point-in-time 保留(period_end<=as_of_date),只統一公式不改視角(L26)。
--
-- 驗證基準(非 byte 等價,是刻意修正):修後 score_universe_at(today)
--   每檔 fund_count_pos == v_factor_scores(全 universe 0 不一致),
--   2376/2492 fund_peg_low 對齊。0.5 的 adj close*adj_factor 維持不動。
...
combined as (
  select
    ...
    p.mom_breakout, p.mom_above_ma200,
...
scored as (
  select
    ...
    coalesce(case when c.mom_ma_golden = 1 then 1 else 0 end, 0)
      ...
      + coalesce(case when c.mom_above_ma200 = 1 then 1 else 0 end, 0) as mom_count_pos_v,
    (case when c.mom_ma_golden is not null then 1 else 0 end
      ...
      + case when c.mom_above_ma200 is not null then 1 else 0 end) as mom_count_total_v,
    -- rev 2
...
comment on function public.score_universe_at(date) is
  'M9.4 階段 3 Step 0:fund_peg_low 對齊 v_stock_score(median 4Q YoY 三層
   fallback PEG),修正 L32 違反。技術因子取價維持 close*adj_factor(0.5)。
   對齊 v_factor_scores v4 + v_stock_rank v4(19 factor)。point-in-time 保留。
   stable function 內讀 app_settings(peg 1.5 / roe 10 / 4 weights)。';
