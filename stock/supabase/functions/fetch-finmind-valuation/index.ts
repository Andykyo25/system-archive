...
Deno.serve(async (req: Request) => {
  ...
  // ETF 沒有本益比/淨值比 —— TaiwanStockPER 對 00xxxx 一律回空陣列。
  // 實測 stock_pe_pb_daily 至今 ETF 列數 = 0、個股 36614 列,即 v_fetch_universe(594)
  // 裡的 416 檔 ETF 每天燒 416 次 call 換 0 列,而且這支排在 institutional(09:00)、
  // lending(09:10) 前面 → 配額吃光後面全餓死(2026-08-11 診斷)。
  // 改讀 v_fetch_universe_stocks(stock 底集,不含 ETF),與籌碼 4 EF 同一個底集。
  // fallback:view 異常 → 退回 holdings/watchlist/industry/universe query(零退化)。
  const fu = await supabase.from("v_fetch_universe_stocks").select("symbol");
  const targetSymbols = new Set<string>();
  if (fu.error) {
    const [holdings, watchlist, industry, universe] = await Promise.all([
